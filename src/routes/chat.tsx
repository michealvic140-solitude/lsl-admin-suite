...
function Room({ room, muted }: { room: Room; muted: boolean }) {
  ...
  async function send(e: React.FormEvent) {
    ...
    if (editing) {
      const { error } = await (supabase.from("chat_messages") as any).update({ content: text.trim(), edited_at: new Date().toISOString() }).eq("id", editing.id);
      if (error) toast.error(error.message); else { setEditing(null); setText(""); }
      return;
    }
    const { error } = await (supabase.from("chat_messages") as any).insert({ user_id: user.id, room, content: text.trim(), reply_to_id: replyTo?.id ?? null });
  ...
  async function pickImage(file: File) {
    ...
    await (supabase.from("chat_messages") as any).insert({ user_id: user.id, room, image_url: publicUrl, reply_to_id: replyTo?.id ?? null });
    setReplyTo(null);
  }
